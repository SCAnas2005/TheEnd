...
public class MachineGun(Rectangle rect, string name, Map map, IWeaponUser owner, int dx = 1, bool dropped = true, bool infiniteAmmo = false, bool debug = false) : Weapon(rect, "", name, map, owner, dx, dropped, infiniteAmmo: infiniteAmmo, debug)
{
    ...
    private float SHOOT_COOLDOWN = 100f;
    ...
    public override void Load(ContentManager Content)
    {
        ...
        var texture = Content.Load<Texture2D>("Weapons/weapons");
        _texture = Utils.ExtractSubTexture(Globals.Graphics.GraphicsDevice, texture, new Rectangle(50, 16, 50, 16));
    }
    ...
    public override void Update(GameTime gameTime)
    {
        ...
        else
        {
            ...
            IsShooting = InputManager.IsHolding(Keys.Space);
            if (IsShooting && player.CanUseItem)
            {
                _shootTimer += (float)gameTime.ElapsedGameTime.TotalMilliseconds;
